...
func TestListRuns_Pagination_WithSortingOnRuntimeDetails(t *testing.T) {
	...
	for _, r := range runs {
		r.RunDetails.StateHistory = []*model.RuntimeStatus{{UpdateTimeInSec: r.RunDetails.CreatedAtInSec, State: model.RuntimeStateSucceeded}}
	...
	assert.Equal(t, model.LargeText("z"), page1[0].RunDetails.PipelineRuntimeManifest) // should project actual value
	...
	assert.Equal(t, model.LargeText("m"), page2[0].RunDetails.PipelineRuntimeManifest)
	...
	assert.Equal(t, model.LargeText("a"), page3[0].RunDetails.PipelineRuntimeManifest)
